...
function Header() {
  ...
  return (
    <header className="sticky top-0 left-0 w-full bg-[#53565A] text-white z-20">
      <div className="flex justify-between items-center h-12 px-4 md:px-7">
        <button
        ...
        <nav className="hidden md:block">
          <ul className="flex items-center space-x-6 px-10 text-xl">
            {navLinks.map((link) => (
              <li key={link.name}>
                <Link to={link.path} className="hover:text-gray-300 transition-colors">{link.name}</Link>
              </li>
            ))}
          </ul>
        </nav>
      ...
      >
        ...
        <div className="absolute bottom-8 left-0 right-0 flex flex-col px-6 space-y-4">
          <Link to="/auth">
            <button className="border border-white px-2 py-1 rounded hover:bg-white hover:text-black transition-colors">
              Вход
            </button>
          </Link>
          <Link to="/auth">
            <button className="bg-[#FF6600] px-2 py-1 rounded hover:bg-orange-800 transition-colors">
              Регистрация
            </button>
          </Link>
        </div>
